...
//                })
                .successHandler(new AuthenticationSuccessHandler() {
                    @Override
                    public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {
                        // 获取登录成功后要跳转的URL，这个URL是在用户注册成功后设置的
                        ...
                        if (savedRequest != null) {
                            ...
                            // 如果提供了redirectUrl参数，即用户注册成功后设置的跳转URL，则使用它进行跳转
                            httpServletResponse.sendRedirect(redirectUrl);
                        } else {
                            // 如果没有提供redirectUrl参数，即用户直接登录，则根据用户角色分别重定向
                            Collection<? extends GrantedAuthority> authorities = authentication.getAuthorities();
                            boolean isAdmin = authorities.contains(new SimpleGrantedAuthority("ROLE_ADMIN")); // 注意角色常量的大小写
                            if (isAdmin) {
                                httpServletResponse.sendRedirect("/admin");
                            } else {
                                httpServletResponse.sendRedirect("/");
